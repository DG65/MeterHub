...
![Modul Version](https://img.shields.io/badge/Modul_Version-0.26.2--beta.1-blue)
...
| **Meteocontrol blue'Log SCADA – Datenlogger (Adresse 97)** | Wirkleistung AC als **Summe aller Wechselrichter** (beim blue'Log XC als Master inkl. der Slaves), Gerätetyp-Rückmeldung | **FC 0x03**, Float32 wortgetauscht (CDAB), Register 10000 (P_AC_INV_SUM) unter der festen SCADA-Adresse **97** (= `UnitId`). Rein lesend. Die Suche bietet Adresse 97 bei jedem gefundenen blue'Log automatisch mit an. Live an einer echten Solarpark-Anlage gegen deren eigene Symcon-Abfrage verifiziert. **Kein Zählerstand** — die SCADA-Schnittstelle dokumentiert unter 97 keine Energie; kumulativen Ertrag liefern die einzelnen Wechselrichter (z. B. per MeterHubVirtual summieren). |
