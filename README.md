...
![Modul Version](https://img.shields.io/badge/Modul_Version-0.27.0--beta.1-blue)
...
| **Meteocontrol blue'Log SCADA – Datenlogger (Adresse 97)** | Wirkleistung AC als **Summe aller Wechselrichter** (beim blue'Log XC als Master inkl. der Slaves), **installierte und aktive Wechselrichter**, Gerätetyp-Rückmeldung; optional Leistungsregelung: verfügbare Wirk-/Blindleistung und wirksamer Wirkleistungs-Sollwert (%) | **FC 0x03**, Float32 wortgetauscht (CDAB), Register 10000–10010 und 10100 (P_AC_INV_SUM, PPC_INV_INST/AVAIL, PPC_P/Q_AC_AVAIL, PPC_P_SET_REL; SCADA ≥ 2.25 laut „SCADA Interface Register V2.27.0") unter der festen SCADA-Adresse **97** (= `UnitId`). Rein lesend. Die Suche bietet Adresse 97 bei jedem gefundenen blue'Log automatisch mit an. Live an einer echten Solarpark-Anlage gegen deren eigene Symcon-Abfrage verifiziert. **Kein Zählerstand im Gerät** — die SCADA-Schnittstelle dokumentiert unter 97 keine Energie. MeterHub rechnet deshalb **„Ertrag gesamt (hochgerechnet)"** aus der Park-Leistung hoch (Leistung × Zeit, Trapez zwischen zwei Messwerten, Lücken über 5 Abfragetakte bzw. mindestens 5 Minuten werden nicht überbrückt, negative Nachtwerte zählen als 0) und archiviert ihn wie einen Zählerstand. Genauer als die Hochrechnung ist die Summe der Wechselrichter-Zähler (E_TOTAL, z. B. per MeterHubVirtual). |
